...
The persistent trust root is `installer/update-public-key.txt`. Its private key is stored in the repository's encrypted `RACKS_UPDATE_PRIVATE_KEY` Actions secret; the initial maintainer copy is outside the checkout at `%LOCALAPPDATA%\RacksReleaseKeys`, with restricted user/SYSTEM access and a DPAPI backup. Back up the private key securely before replacing that machine. Do not regenerate it for subsequent releases.

The `Windows signed beta` workflow builds the version-matching `v2.*-beta.*` tag, tests both source and the published package in isolated profiles, signs the installer/feed, and creates a **draft** prerelease with checksums. It never promotes a draft automatically. Once the beta is published and its asset signatures checked, publish its `appcast.xml` and `.signature` together under `feeds/beta/win-x64` on `codex/release-feeds`. `RACKS_UPDATE_FEED_BASE` selects that branch's public HTTPS feed root; stable feeds must use a separate `stable` directory.

The first Windows beta, `v2.0.0-beta.1`, is withdrawn and retained as a draft. Its beta feed is now signed and empty; see [Native recovery](NATIVE-RECOVERY.md). Its public installer and feed were independently verified after publication, including a real NetSparkle HTTPS download. Preserve published version tags and assets; corrections require a new version. Keep the beta feed separate from stable, and publish feed bytes without changing their encoding or line endings after signing.
