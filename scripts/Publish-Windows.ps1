...
$releaseOutput = if ($OutputDirectory) { [IO.Path]::GetFullPath($OutputDirectory) } else { Join-Path $repoRoot ('.artifacts/release-' + [guid]::NewGuid().ToString('N')) }
if ((Test-Path -LiteralPath $releaseOutput) -and @(Get-ChildItem -LiteralPath $releaseOutput -Force).Count) { throw 'Use a new or empty release output directory.' }
...
try {
    ...
    & "$PSScriptRoot/Test-Desktop.ps1" -Dotnet $Dotnet -AssemblyPath (Join-Path $repoRoot '.artifacts/publish/win-x64/Racks.Next.dll')
    ...
    $feedOutput = Join-Path $releaseOutput 'win-x64'
    ...
    Copy-Item -LiteralPath $package -Destination $releaseOutput
